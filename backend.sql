...
def studentdetails():
    query = db.engine.execute("SELECT * FROM student")
    return render_template('studentdetails.html', query=query)

@app.route('/triggers')
def triggers():
    query = db.engine.execute("SELECT * FROM trig")
...
def edit(id):
    ...
    if request.method == "POST":
        ...
        db.engine.execute(f"UPDATE student SET rollno='{rollno}', sname='{sname}', sem='{sem}', gender='{gender}', branch='{branch}', email='{email}', number='{num}', address='{address}'")
...
def signup():
    if request.method == "POST":
        ...
        db.engine.execute(f"INSERT INTO user (username, email, password) VALUES ('{username}', '{email}', '{encpassword}')")
